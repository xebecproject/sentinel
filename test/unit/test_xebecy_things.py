...
def test_valid_xebec_address():
    from xebeclib import is_valid_xebec_address

    main = valid_xebec_address()
    test = valid_xebec_address('testnet')

    assert is_valid_xebec_address(main) is True
    assert is_valid_xebec_address(main, 'mainnet') is True
    assert is_valid_xebec_address(main, 'testnet') is False

    assert is_valid_xebec_address(test) is False
    assert is_valid_xebec_address(test, 'mainnet') is False
    assert is_valid_xebec_address(test, 'testnet') is True


def test_invalid_xebec_address():
    from xebeclib import is_valid_xebec_address

    main = invalid_xebec_address()
    test = invalid_xebec_address('testnet')

    assert is_valid_xebec_address(main) is False
    assert is_valid_xebec_address(main, 'mainnet') is False
    assert is_valid_xebec_address(main, 'testnet') is False

    assert is_valid_xebec_address(test) is False
    assert is_valid_xebec_address(test, 'mainnet') is False
    assert is_valid_xebec_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from xebeclib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % xebeclib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(xebeclib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(xebeclib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(xebeclib.blocks_to_seconds(16616)) == 2612035
