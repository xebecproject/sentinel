...
class XebecDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some xebecd methods
    ...
    @classmethod
    def from_xebec_conf(self, xebec_dot_conf):
        from xebec_config import XebecConfig
        config_text = XebecConfig.slurp_config_file(xebec_dot_conf)
        creds = XebecConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = xebeclib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = xebeclib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = xebeclib.blocks_to_seconds(diff)
