...
def is_valid_xebec_address(address, network='mainnet'):
    ...
    xebec_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Xebec address
        return False

    if (address_version != xebec_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('xebec.conf')
