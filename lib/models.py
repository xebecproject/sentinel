...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, xebecd):
        golist = xebecd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_xebecd(xebecd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_xebecd(self, xebecd, rec):
        import decimal
        import xebeclib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from xebecd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from xebecd, with every run
        ...
        # get/create, then sync payment amounts, etc. from xebecd - Xebecd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(xebecd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from xebecd! %s" % e)
            govobj.vote_delete(xebecd)
            return (govobj, None)
    ...
    def vote_delete(self, xebecd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(xebecd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, xebecd, signal, outcome):
        import xebeclib
        ...
        output = xebecd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = xebeclib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(xebecd, signal)

    def sync_network_vote(self, xebecd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = xebecd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = XEBECD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 xebec addr, non-multisig
            if not xebeclib.is_valid_xebec_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Xebec address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Xebec Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (xebeclib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = XEBECD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not xebeclib.is_valid_xebec_address(addr, config.network):
    ...
    def hash(self):
        import xebeclib
        return xebeclib.hashit(self.serialise())
