...
sentinel_cfg = XebecConfig.tokenize(sentinel_config_file)
sentinel_version = "1.4.0"


def get_xebec_conf():
    if sys.platform == 'win32':
        xebec_conf = os.path.join(os.getenv('APPDATA'), "XebecCore/xebec.conf")
    else:
        home = os.environ.get('HOME')

        xebec_conf = os.path.join(home, ".xebeccore/xebec.conf")
        if sys.platform == 'darwin':
            xebec_conf = os.path.join(home, "Library/Application Support/XebecCore/xebec.conf")

    xebec_conf = sentinel_cfg.get('xebec_conf', xebec_conf)
