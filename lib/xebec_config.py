...
class XebecConfig():
    ...
    def slurp_config_file(self, filename):
        # read xebec.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is xebec.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from xebec.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Xebec defaults...
        default_port = 28280 if (network == 'mainnet') else 28281

        # use default port for network if not specified in xebec.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from xebec.conf
